 ...
 * 作者：ywzou <br>
 * 创建时间：2018年1月18日 <br>
 * 描述：协议拟制任务监听 穿件后 create
 */
@Component("createConAuditTaskListener")
public class CreateConAuditTaskListener implements TaskListener {
	...
	public void notify(DelegateTask task) {
		...
		if (StringUtils.isNotBlank(applyId)) {
			...
			sql.append("and hp.ID_=#{procInstId} and hi.TASK_DEF_KEY_='makeCon' ");
			sql.append("order by END_TIME_ desc ) where rownum=1");
			
			HistoricTaskInstance historicTaskInstance = historyService
					.createNativeHistoricTaskInstanceQuery()
					.sql(sql.toString())
					.parameter("applyId", applyId)
					.parameter("procInstId", task.getProcessInstanceId()).singleResult();
