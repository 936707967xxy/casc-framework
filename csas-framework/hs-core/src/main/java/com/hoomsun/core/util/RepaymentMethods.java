...
public class RepaymentMethods {
	...
	public static SysContract computeRepaymentCon(BigDecimal auditMoney, SysProduct product) {
		...
		case 1:// 等额本息
			...
			// 月还款额 月还款额 = [贷款本金×月利率×（1+月利率）^还款月数]÷[（1+月利率）^还款月数－1]
			BigDecimal payMonth = BigDecimal.ZERO;

			// 【不按期】【预收】【作为本金】 现有业务方式
			BigDecimal baqyszbjMonthPay = fee.getBaqyszbj();
			// 批核月还
			payMonth = getMonthPay(auditMoney.add(baqyszbjMonthPay.multiply(bigSterm, mc)), sterm, monthRate);

			// 合同金额
			BigDecimal conAmount = scale(auditMoney.add(feeSum.multiply(bigSterm)));
	...
	public static Fee buildFee(BigDecimal auditMoney, List<SysProductRate> productRates, BigDecimal sterm) {
		Fee fee = new RepaymentMethods().new Fee();
	...
	public static void setBillsDateRanges(List<BillsDateRange> billsDateRanges) {
		RepaymentMethods.billsDateRanges = billsDateRanges;
	}
